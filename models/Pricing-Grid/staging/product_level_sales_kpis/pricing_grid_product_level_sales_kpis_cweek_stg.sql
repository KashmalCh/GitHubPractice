...
SELECT DISTINCT
    ...
    ,((SUM(SnWrSales)-COALESCE(SUM(SnWrCoreCost),0))+COALESCE(SUM(SnWrVI),0)) AS TC_D
...
WHERE 
	...
	AND FWeek = (SELECT FWeekNo FROM `mfrm_sales.dim_calendar` WHERE CalendarDate = CURRENT_DATE())
