...
SELECT DISTINCT
    ...
    ,((((SUM(SnWrSales)-COALESCE(SUM(SnWrCoreCost),0))+COALESCE(SUM(SnWrVI),0)))/(CASE WHEN SUM(SnWrSales) = 0 THEN 1 ELSE SUM(SnWrSales) END)) AS TC_P
    ,(
    ...
    ,((SUM(SnWrSales)-COALESCE(SUM(SnWrCoreCost),0))+COALESCE(SUM(SnWrVI),0)/CASE WHEN SUM(B.Sqft)=0 THEN 1 ELSE SUM(B.Sqft) END) AS CyPPSF
    ,((SUM(LySnWrSales)-COALESCE(SUM(LySnWrCoreCost),0))+COALESCE(SUM(SnWrVI),0)/CASE WHEN SUM(B.Sqft)=0 THEN 1 ELSE SUM(B.Sqft) END) AS LyPPSF
    ...
    FROM {{ref('pricing_grid_sales_update')}} A
LEFT JOIN 
	(
        SELECT 
			ProductID
		...
		FROM(
			SELECT
				...
				END AS ValueZone
			FROM 
                {{source('mfrm_sales_dynamic', 'fact_day_iat_inventory')}} A
			LEFT JOIN (
				SELECT
					A.OrderBrand
					,A.ModelName
			...
			ON 
                A.PrimaryVendorID = B.OrderBrand
