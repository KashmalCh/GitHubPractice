...
SELECT
    ...
    FROM(
        ...
        AND CAST(SUBSTR(REPLACE(REPLACE(REPLACE(FYearWeek,'_',''),'FY','20'),'Wk',''),5,6) AS INT64) = (SELECT FWeekNo FROM `mfrm_sales.dim_calendar` WHERE CalendarDate = CURRENT_DATE())
        ) A
LEFT JOIN {{ref('pricing_grid_product_level_sales_kpis_cweek_stg')}} B
