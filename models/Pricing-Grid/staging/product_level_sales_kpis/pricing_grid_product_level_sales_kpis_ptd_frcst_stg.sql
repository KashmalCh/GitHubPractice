...
SELECT
    ...
    FROM(
        ...
        AND CAST(SUBSTR(REPLACE(REPLACE(REPLACE(FYearWeek,'_',''),'FY','20'),'Wk',''),5,6) AS INT64) 
        IN (SELECT DISTINCT FWeekNo FROM {{source('mfrm_sales_dynamic', 'dim_calendar')}} WHERE FYearNo = (SELECT FYearNo FROM `mfrm_sales.dim_calendar` WHERE CalendarDate = CURRENT_DATE()) AND FPeriodNo = (SELECT FPeriodNo FROM `mfrm_sales.dim_calendar` WHERE CalendarDate = CURRENT_DATE()))
        ) A
LEFT JOIN {{ref('pricing_grid_product_level_sales_kpis_ptd_stg')}} B
