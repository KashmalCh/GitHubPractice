...
SELECT
    ...
    FROM(
        ...
        IN (SELECT DISTINCT FWeekNo FROM {{source('mfrm_sales_dynamic', 'dim_calendar')}} WHERE FYearNo = (SELECT FYearNo FROM `mfrm_sales.dim_calendar` WHERE CalendarDate = CURRENT_DATE()) AND FQtrNo = (SELECT FQtrNo FROM `mfrm_sales.dim_calendar` WHERE CalendarDate = CURRENT_DATE()))
        ) A
LEFT JOIN {{ref('pricing_grid_product_level_sales_kpis_qtd_stg')}} B
