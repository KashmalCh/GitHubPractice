...
FROM (
        SELECT DISTINCT
            ...
            ROW_NUMBER() OVER (PARTITION BY `mattress-firm-inc.mfrm_customer_360.fn_normalize_email`(EmailAddr) ORDER BY COALESCE(UpdatedDate,CreatedDate) DESC,CustomerId  DESC) as CustomerId_Num
        FROM (    
                SELECT
                    UpdatedDate,
                    CreatedDate,
